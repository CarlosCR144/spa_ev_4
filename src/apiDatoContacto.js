...
export const getDatosContacto = async () => {
  const { data } = await axios.get(API_URL_DATO_CONTACTO, {
    headers: {
      apikey: API_KEY,
...
export const deleteDatosContacto = async (id_dato_contacto) => {
  await axios.delete(`${API_URL_DATO_CONTACTO}?id_contacto=eq.${id_dato_contacto}`, {
    headers: {
      apikey: API_KEY,
      Authorization: `Bearer ${API_KEY}`,
    },
